...
# https://docs.sencha.com/cmd/7.5.1/guides/getting_started_cmd.html#getting_started_cmd_-_dependencies
RUN apt-get update \
 && apt-get upgrade -y \
...
ARG CMD_VERSION=7.5.1.20
RUN wget "http://trials.sencha.com/cmd/${CMD_VERSION%.*}/no-jre/SenchaCmd-$CMD_VERSION-linux-amd64.sh.zip" \
